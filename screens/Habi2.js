...
export default function Habi2({ showCamera, setShowCamera }) {
  const [hasPermission, setHasPermission] = useState(null);
  const [cameraReady, setCameraReady] = useState(false);
 // const [showCamera, setShowCamera] = useState(false);
  const [photo12, setPhoto12] = useState(null);
  const [photo22, setPhoto22] = useState(null);
  ...
  useEffect(() => {
    (async () => {
      try {
        const photo12Uri = await FileSystem.getInfoAsync(`${FileSystem.documentDirectory}photo12.jpg`);
        const photo22Uri = await FileSystem.getInfoAsync(`${FileSystem.documentDirectory}photo22.jpg`);
        
        if (photo12Uri.exists) {
          const photo12Base64 = await FileSystem.readAsStringAsync(`${FileSystem.documentDirectory}photo12.jpg`, { encoding: FileSystem.EncodingType.Base64 });
          setPhoto12(`data:image/jpg;base64,${photo12Base64}`);
          setCurrentPhoto(`data:image/jpg;base64,${photo12Base64}`);
        }

        if (photo22Uri.exists) {
          const photo22Base64 = await FileSystem.readAsStringAsync(`${FileSystem.documentDirectory}photo22.jpg`, { encoding: FileSystem.EncodingType.Base64 });
          setPhoto22(`data:image/jpg;base64,${photo22Base64}`);
        }
  ...
  const handleTakePhoto = async () => {
    if (!photo12) {
      ...
      if (photoBase64) {
        const photoUri = `${FileSystem.documentDirectory}photo12.jpg`;
        await FileSystem.writeAsStringAsync(photoUri, photoBase64, { encoding: FileSystem.EncodingType.Base64 });
        setPhoto12(`data:image/jpg;base64,${photoBase64}`);
        setCurrentPhoto(`data:image/jpg;base64,${photoBase64}`);
      }
    } else if (!photo22) {
      const photoBase64 = await takePicture();
      if (photoBase64) {
        const photoUri = `${FileSystem.documentDirectory}photo22.jpg`;
        await FileSystem.writeAsStringAsync(photoUri, photoBase64, { encoding: FileSystem.EncodingType.Base64 });
        setPhoto22(`data:image/jpg;base64,${photoBase64}`);
        setCurrentPhoto(`data:image/jpg;base64,${photoBase64}`);
      }
      setShowCamera(false);
    }
  ...
  const delante =async () => {
    await FileSystem.deleteAsync(`${FileSystem.documentDirectory}photo12.jpg`);
    setPhoto12(null);
    setCurrentPhoto(photo22 ? photo22 : null);
}
    ...
    [
      ...
      },
      {
        ...
        onPress: async () => {
            ...
            await FileSystem.deleteAsync(`${FileSystem.documentDirectory}photo22.jpg`);
            setPhoto22(null);
            setCurrentPhoto(null);
        },
      },
      ...
      <TouchableOpacity onPress={handleTakePhoto} style={styles.circleButton}>
        ...
        <View style={styles.imageContainer}>
          ...
          {currentPhoto ? (
            <>
             <>
             <Text style={styles.super3}>Habilitacion 2</Text>
             <Text style={styles.textAyuda2}>toca la imagen para ampliar</Text>
             </>
            <>
            <TouchableOpacity onPress={() => openModal([photo12, photo22])}>
              <Image source={{ uri: photo12 }} style={styles.image} />
            </TouchableOpacity>
            <TouchableOpacity onPress={() => openModal([photo12, photo22])}>
              <Image source={{ uri: photo22 }} style={styles.image} />
            </TouchableOpacity>
            <ImageModal 
