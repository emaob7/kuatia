...
export default function Reg2({ showCamera, setShowCamera }) {
  const [hasPermission, setHasPermission] = useState(null);
  const [cameraReady, setCameraReady] = useState(false);
 // const [showCamera, setShowCamera] = useState(false);
  const [photo32, setPhoto32] = useState(null);
  const [photo42, setPhoto42] = useState(null);
  ...
  const handleTakePhoto = async () => {
    if (!photo32) {
      ...
      if (photoBase64) {
        const photoUri = `${FileSystem.documentDirectory}photo32.jpg`;
        await FileSystem.writeAsStringAsync(photoUri, photoBase64, { encoding: FileSystem.EncodingType.Base64 });
        setPhoto32(`data:image/jpg;base64,${photoBase64}`);
        setCurrentPhoto(`data:image/jpg;base64,${photoBase64}`);
      }
    } else if (!photo42) {
      const photoBase64 = await takePicture();
      if (photoBase64) {
        const photoUri = `${FileSystem.documentDirectory}photo42.jpg`;
        await FileSystem.writeAsStringAsync(photoUri, photoBase64, { encoding: FileSystem.EncodingType.Base64 });
        setPhoto42(`data:image/jpg;base64,${photoBase64}`);
        setCurrentPhoto(`data:image/jpg;base64,${photoBase64}`);
      }
      setShowCamera(false);
    }
      ...
      <TouchableOpacity onPress={handleTakePhoto} style={styles.circleButton}>
        ...
        <View style={styles.imageContainer}>
          ...
          {currentPhoto ? (
            <>
             <>
             <Text style={styles.super3}>Licencia 2</Text>
             <Text style={styles.textAyuda2}>toca la imagen para ampliar</Text>
             </>
            <>
            <TouchableOpacity onPress={() => openModal([photo32, photo42])}>
              <Image source={{ uri: photo32 }} style={styles.image} />
            </TouchableOpacity>
            <TouchableOpacity onPress={() => openModal([photo32, photo42])}>
              <Image source={{ uri: photo42 }} style={styles.image} />
            </TouchableOpacity>
            <ImageModal 
