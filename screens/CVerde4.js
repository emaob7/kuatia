...
export default function CVerde4({ showCamera, setShowCamera }) {
  const [hasPermission, setHasPermission] = useState(null);
  const [cameraReady, setCameraReady] = useState(false);
 // const [showCamera, setShowCamera] = useState(false);
  const [photo74, setPhoto74] = useState(null);
  const [photo84, setPhoto84] = useState(null);
  ...
  useEffect(() => {
    (async () => {
      try {
        const photo74Uri = await FileSystem.getInfoAsync(`${FileSystem.documentDirectory}photo74.jpg`);
        const photo84Uri = await FileSystem.getInfoAsync(`${FileSystem.documentDirectory}photo84.jpg`);
        
        if (photo74Uri.exists) {
          const photo7Base64 = await FileSystem.readAsStringAsync(`${FileSystem.documentDirectory}photo74.jpg`, { encoding: FileSystem.EncodingType.Base64 });
          setPhoto74(`data:image/jpg;base64,${photo74Base64}`);
          setCurrentPhoto(`data:image/jpg;base64,${photo74Base64}`);
        }

        if (photo84Uri.exists) {
          const photo84Base64 = await FileSystem.readAsStringAsync(`${FileSystem.documentDirectory}photo84.jpg`, { encoding: FileSystem.EncodingType.Base64 });
          setPhoto84(`data:image/jpg;base64,${photo84Base64}`);
        }
  ...
  const handleTakePhoto = async () => {
    if (!photo74) {
      ...
      if (photoBase64) {
        const photoUri = `${FileSystem.documentDirectory}photo74.jpg`;
        await FileSystem.writeAsStringAsync(photoUri, photoBase64, { encoding: FileSystem.EncodingType.Base64 });
        setPhoto74(`data:image/jpg;base64,${photoBase64}`);
        setCurrentPhoto(`data:image/jpg;base64,${photoBase64}`);
      }
    } else if (!photo84) {
      const photoBase64 = await takePicture();
      if (photoBase64) {
        const photoUri = `${FileSystem.documentDirectory}photo84.jpg`;
        await FileSystem.writeAsStringAsync(photoUri, photoBase64, { encoding: FileSystem.EncodingType.Base64 });
        setPhoto84(`data:image/jpg;base64,${photoBase64}`);
        setCurrentPhoto(`data:image/jpg;base64,${photoBase64}`);
      }
      setShowCamera(false);
    }
...
const delante =async () => {
    await FileSystem.deleteAsync(`${FileSystem.documentDirectory}photo74.jpg`);
    setPhoto74(null);
    setCurrentPhoto(photo84 ? photo84 : null);
}
    ...
    [
      ...
      },
      {
        ...
        onPress: async () => {
            ...
            await FileSystem.deleteAsync(`${FileSystem.documentDirectory}photo84.jpg`);
            setPhoto84(null);
            setCurrentPhoto(null);
        },
      },
    ...
    onPress={delante}  >
      ...
      <TouchableOpacity onPress={handleTakePhoto} style={styles.circleButton}>
        ...
        <View style={styles.imageContainer}>
          ...
          {currentPhoto ? (
            <>
             <>
             <Text style={styles.super3}>Cedula Verde 4</Text>
             <Text style={styles.textAyuda2}>toca la imagen para ampliar</Text>
             </>
            <>
            <TouchableOpacity onPress={() => openModal([photo74, photo84])}>
              <Image source={{ uri: photo74 }} style={styles.image} />
            </TouchableOpacity>
            <TouchableOpacity onPress={() => openModal([photo74, photo84])}>
              <Image source={{ uri: photo84 }} style={styles.image} />
            </TouchableOpacity>
            <ImageModal 
     ...
     />
            ...
            <>
             ...
             <Text style={styles.super2}>Cedula Verde 4</Text>
