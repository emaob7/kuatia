...
export default function Reg4({ showCamera, setShowCamera }) {
  const [hasPermission, setHasPermission] = useState(null);
  const [cameraReady, setCameraReady] = useState(false);
 // const [showCamera, setShowCamera] = useState(false);
  const [photo34, setPhoto34] = useState(null);
  const [photo44, setPhoto44] = useState(null);
  ...
  useEffect(() => {
    (async () => {
      try {
        const photo34Uri = await FileSystem.getInfoAsync(FileSystem.documentDirectory + 'photo34.jpg');
        const photo44Uri = await FileSystem.getInfoAsync(FileSystem.documentDirectory + 'photo44.jpg');
        
        if (photo34Uri.exists) {
          const photo34Base64 = await FileSystem.readAsStringAsync(FileSystem.documentDirectory + 'photo34.jpg', { encoding: FileSystem.EncodingType.Base64 });
          setPhoto34(`data:image/jpg;base64,${photo34Base64}`);
          setCurrentPhoto(`data:image/jpg;base64,${photo34Base64}`);
        }

        if (photo44Uri.exists) {
          const photo44Base64 = await FileSystem.readAsStringAsync(FileSystem.documentDirectory + 'photo44.jpg', { encoding: FileSystem.EncodingType.Base64 });
          setPhoto44(`data:image/jpg;base64,${photo44Base64}`);
        }
  ...
  const handleTakePhoto = async () => {
    if (!photo34) {
      ...
      if (photoBase64) {
        const photoUri = `${FileSystem.documentDirectory}photo34.jpg`;
        await FileSystem.writeAsStringAsync(photoUri, photoBase64, { encoding: FileSystem.EncodingType.Base64 });
        setPhoto34(`data:image/jpg;base64,${photoBase64}`);
        setCurrentPhoto(`data:image/jpg;base64,${photoBase64}`);
      }
    } else if (!photo44) {
      const photoBase64 = await takePicture();
      if (photoBase64) {
        const photoUri = `${FileSystem.documentDirectory}photo44.jpg`;
        await FileSystem.writeAsStringAsync(photoUri, photoBase64, { encoding: FileSystem.EncodingType.Base64 });
        setPhoto44(`data:image/jpg;base64,${photoBase64}`);
        setCurrentPhoto(`data:image/jpg;base64,${photoBase64}`);
      }
      setShowCamera(false);
    }
  ...
  const delante =async () => {
    await FileSystem.deleteAsync(FileSystem.documentDirectory + 'photo34.jpg');
    setPhoto34(null);
    setCurrentPhoto(photo44 ? photo44 : null);
}
    ...
    [
      ...
      },
      {
        ...
        onPress: async () => {
            ...
            await FileSystem.deleteAsync(FileSystem.documentDirectory + 'photo44.jpg');
            setPhoto44(null);
            setCurrentPhoto(null);
        },
      },
    ...
    onPress={delante}  >
      ...
      <TouchableOpacity onPress={handleTakePhoto} style={styles.circleButton}>
        ...
        <View style={styles.imageContainer}>
          ...
          {currentPhoto ? (
            <>
             <>
             <Text style={styles.super3}>Licencia 4</Text>
             <Text style={styles.textAyuda2}>toca la imagen para ampliar</Text>
             </>
            <>
            <TouchableOpacity onPress={() => openModal([photo34, photo44])}>
              <Image source={{ uri: photo34 }} style={styles.image} />
            </TouchableOpacity>
            <TouchableOpacity onPress={() => openModal([photo34, photo44])}>
              <Image source={{ uri: photo44 }} style={styles.image} />
            </TouchableOpacity>
            <ImageModal 
     ...
     />
            ...
            <>
             ...
             <Text style={styles.super2}>Licencia 4</Text>
